...
```text
NtWriteVirtualMemory 能不能修改目标进程？
能。

NtWriteVirtualMemory 能不能单独让新写入的内容执行？
通常不能。
```

`NtWriteVirtualMemory` 的职责是把一段数据复制到目标进程的地址空间。它本身不负责：
...
因此，**向新分配的私有内存写入 shellcode 后，通常还需要额外的执行触发步骤**。如果没有任何线程、回调或既有执行路径到达这块地址，CPU 不会因为那里出现了数据就自动执行它。

### 什么时候“只写内存”也可能生效

“只写内存”可以成立，但这里的“只写”是指远程操作中只观察到内存写入这一类动作，并不代表目标进程完全没有其他前置条件。典型前提包括：

- 目标位置本来就在原有执行路径上，例如目标进程之后必然会调用的函数、回调或分发表项；
- 写入位置已经具备合适的写权限，或者此前已经由其他机制准备好；
- 写入内容能够直接改变既有控制流，且不会破坏指令边界、线程状态或程序所需的数据结构；
- 目标进程原有线程稍后会自然经过该位置。
...
目标进程的定时任务
  ↓
每隔一段时间调用 CheckUpdate()

攻击者对目标进程执行 NtWriteVirtualMemory
  ↓
修改 CheckUpdate() 的实现或其可写的回调指针
  ↓
目标进程原有线程下一次执行定时任务
  ↓
进入被修改后的逻辑
...
这个例子里，攻击者没有新建远程线程，也没有显式修改线程上下文；执行机会来自目标进程原本就存在的调用路径。类似思路也可能涉及函数指针、回调表、分发表或已加载模块中的控制数据。

但它不是“`NtWriteVirtualMemory` 天然等于执行”。如果目标代码页不可写，写入可能失败；如果写入的是一块从未被执行流引用的新内存，则仍然需要远程线程、APC、线程劫持、回调触发等后续动作。现代系统的 DEP、CFG、CIG、ACG 等机制也可能进一步影响成功率。

### 分析时的判断原则

看到 `NtWriteVirtualMemory` 后，不要只问“有没有 `CreateRemoteThread`”，而要继续回答：

1. 写入的目标地址属于代码页、数据页、私有页还是共享映射？
2. 该地址是否已经被目标进程的线程、回调或间接调用路径引用？
3. 写入后是否发生了页面权限变化、线程上下文变化、APC 排队或异常执行？
4. 写入内容是参数和配置，还是代码、函数指针、回调地址等控制流相关数据？

所以，单独一个 `NtWriteVirtualMemory` 既不能直接证明注入，也不能因为没有远程线程就排除注入。关键在于**写入对象与后续执行证据之间是否能闭合**。
...
| 阶段 | 解决的问题 | 防守侧要看的重点 |
|---|---|---|
| `NtOpenProcess` | 获取目标进程句柄 | 调用方、目标进程、请求的访问权限和完整性级别 |
| `NtAllocateVirtualMemory` | 准备目标地址空间中的存储区域 | `MEM_PRIVATE` 区域、初始页面权限和分配时机 |
| `NtWriteVirtualMemory` | 把载荷、路径、参数或引导数据写入目标 | 写入地址、大小、内容类型和来源进程 |
| `NtProtectVirtualMemory` | 在需要时调整页面保护属性 | `RW → RX`、`RWX`、是否存在合理的 JIT 或兼容性场景 |
| `NtCreateThreadEx` | 让目标进程出现新的执行入口 | 线程起始地址是否落在私有可执行页、调用栈和线程归属 |

高层 Win32 API 的等价抽象通常写成：
...
这里的“等价”是能力层面的对应，不要求每个样本严格按照同样的 API 名称实现。样本也可能用节区映射、线程池、APC 或线程上下文操作替换其中某一环。

### 为什么要有这么多步骤

因为经典链路通常面对的是一块**目标进程原本没有的全新载荷**：
...
而直接覆盖既有代码或控制数据，依赖目标进程原本就有合适的执行路径，兼容性、稳定性和可控性都更差。目标代码页常常是只读可执行，覆盖后还可能影响正在运行的其他线程；相比之下，准备独立内存并显式安排执行入口，更适合承载较大的独立载荷。
...
|---|---|---|---|
| 直接写内存 / 执行流篡改 | 修改既有代码、函数指针、回调或分发表；也可能写入已准备好的区域 | 目标进程原有线程自然经过被修改的位置 | 写入地址的内存类型与权限、是否属于控制流数据、写入后是否出现异常执行 |
...
需要特别区分：**直接写内存是一个动作，注入链是多个动作的组合**。如果写入的只是配置、参数或进程间通信数据，它未必构成注入；只有当写入内容改变了目标进程的代码、控制流或执行相关数据，并且后续确实被目标进程使用时，才更接近“通过内存写入实现注入或执行流篡改”。
