...
这是理解贪婪和非贪婪匹配的直观例子。假设字符串是：

~~~text
访问 http://first.example，然后继续访问 http://second.example
~~~

##### 非贪婪：.*?http

`.*?` 会尽可能少地匹配字符。它从当前位置开始一点点扩展，直到后面的 `http` 第一次匹配成功：
...
因此，`.*?http` 通常用于“从当前位置开始，截取到第一个 http 为止”的场景。需要注意，它匹配到的整体结果包含前面的字符和 `http`，并不只是 `http` 本身；如果只想提取关键字，应直接使用 `http`。

##### 贪婪：.*http

`.*` 是贪婪量词，会先尽可能多地吞掉字符。它会先尝试匹配到字符串末尾，然后为了让后面的 `http` 成功，从末尾开始回溯，最终停在最后一个可以匹配的 `http`：

~~~text
正则：.*http
结果：访问 http://first.example，然后继续访问 http
                                      ↑
                                      最后一个 http
~~~
...
所以，在同一个字符串和同一个起始位置上，通常可以这样记忆：

| 正则 | 量词行为 | http 命中的位置 |
| --- | --- | --- |
| `.*?http` | 非贪婪，尽快满足后续条件 | 第一个 `http` |
| `.*http` | 贪婪，先尽量扩展再回溯 | 最后一个 `http` |

这里的“第一个”和“最后一个”是指在同一次匹配、同一个起始位置和同一个文本范围内的结果。正则引擎是否跨越换行，还取决于点号的行为以及是否启用了 DOTALL、`s` 等模式。

##### 与捕获组结合

如果希望只提取目标标记，可以把目标放在捕获组中：
...
如果要匹配第一个 URL，可以使用更明确的字符范围，而不是让点号任意跨越内容：
...
`.*?` 适合快速表达“到第一个目标标记为止”，但在 HTML、日志或嵌套结构中，最好根据实际分隔符使用更精确的字符类；正则表达式并不能完整解析任意嵌套结构。
